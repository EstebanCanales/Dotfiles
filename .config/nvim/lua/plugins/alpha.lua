return {
	...
	config = function()
		...
		local function info_value()
			...
			return surround('') ..
					loaded_plugins .. '/' .. total_plugins .. ' plugins loaded ' .. nvim_version_info
		end
		...
		local buttons = {
			...
			val = {
				...
				button("P", "Projects", ":Neotree ~/WORK/ <CR>",
					{ icon = ' ', hl = { { 'Keyword', 1, 2 }, { 'Normal', 3, 52 } } }
				),
				button("N", "Notes", ":Neotree ~/WORK/NOTES<CR>",
				...
				button("C", "Config", ":Neotree ~/Dotfiles/.config/nvim/lua<CR>",
					{ icon = ' ', hl = { { 'String', 1, 2 }, { 'Normal', 3, 52 } } }
				),
				button("L", "Configure LSP", ":ToggleServer<CR>",
					{ icon = ' ', hl = { { 'String', 1, 2 }, { 'Normal', 3, 52 } } }
				),
				{ type = 'padding', val = 1 },
				button("q", "Quit", ":qa<CR>",
